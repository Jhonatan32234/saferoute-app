...
class HomeRemoteDataSource {
  ...
  Future<List<DestinoRecienteModel>> getDestinosRecientes() async {
    try {
      ...
      if (response.statusCode == 200) {
        final decoded = jsonDecode(response.body);
        // Validamos si la respuesta es un Map con la clave 'destinos' o una lista directa
        if (decoded is Map && decoded.containsKey('destinos')) {
          final List<dynamic> list = decoded['destinos'] ?? [];
          return list.map((json) => DestinoRecienteModel.fromJson(json)).toList();
        } else if (decoded is List) {
          return decoded.map((json) => DestinoRecienteModel.fromJson(json)).toList();
        }
      }
      return [];
    } catch (e) {
      debugPrint('❌ Error en DataSource destinos: $e');
      return [];
    }
  }
